...
class Segments:
    ...
    def get_segment_information(self):
        ...
        for segment in segment_files:
            path = os.path.join(self._segments_folder, segment)
            duration = self.segment_duration(path)
            if not duration:
                continue

            # Need to subtract the segment duration since creation time changes until
            # segment is written completely
            start_time = os.path.getctime(path) - CAMERA_SEGMENT_DURATION
    ...
    def ffmpeg_concat(self, segment_script, file_name):
        """Generate and run GStreamer command to concatenate segments."""
        ffmpeg_cmd = (
            [
                "ffmpeg",
                "-hide_banner",
