...
class GotifyEventNotifier:
    ...
    def _recording_start_event_handler(self, event_data: Event[EventRecorderData]) -> None:
        ...
        global_detection_label = self._config.get(CONFIG_DETECTION_LABEL, CONFIG_DETECTION_LABEL_DEFAULT)
        detection_labels = camera_detection_label if camera_detection_label is not None else global_detection_label
        ...
        send_thumbnail = camera_send_thumbnail if camera_send_thumbnail is not None else global_send_thumbnail
        ...
        if send_thumbnail and thumbnail is not None:
            try:
                ...
                resized_thumbnail = cv2.resize(thumbnail, (new_width, new_height), interpolation=cv2.INTER_AREA)
                
                # Send notification with image
                asyncio.run_coroutine_threadsafe(
                    self._send_notification_with_image(title, message, resized_thumbnail),
                    self._loop
                )
            except Exception as e:
                LOGGER.error(f"Failed to prepare image notification: {e}")
                # Fall back to text-only notification
                asyncio.run_coroutine_threadsafe(
                    self._send_text_notification(title, message),
                    self._loop
                )
    ...
    async def _send_notification_with_image(self, title, message, image):
        ...
        try:
            ...
            LOGGER.error(f"Failed to send Gotify message with image: {e}")
            # Fall back to text-only notification
            await self._send_text_notification(title, message)
    ...
    def _send_gotify_message(self, title, message):
        ...
        if response.status_code != 200:
            LOGGER.error(
                f"Failed to send Gotify message: {response.status_code} - {response.text}"
            )
        else:
            pass
    ...
    def _send_gotify_message_with_image(self, title, message, image):
        ...
        if response.status_code != 200:
            LOGGER.error(
                f"Failed to send Gotify message with image: {response.status_code} - {response.text}"
            )
        else:
            pass
