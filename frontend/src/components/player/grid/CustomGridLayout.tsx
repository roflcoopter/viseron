...
function CameraPlayerItem({ 
  ...
  camera: types.Camera | types.FailedCamera;
  style: any;
  renderPlayer: (camera: types.Camera | types.FailedCamera, playerRef: React.RefObject<any>) => React.ReactElement;
}) {
  ...
  ({ cameras, containerRef: _containerRef, renderPlayer }, ref) => {
    ...
    const { main, sides } = useMemo(() => getCameraPositions(), [getCameraPositions]);
