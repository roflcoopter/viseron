...
function MenuProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleFlipView = useCallback(() => {
    if (contextMenuState.camera) {
      ...
      const currentFlipState =
        usePlayerSettingsStore.getState().flipViewMap[
          contextMenuState.camera.identifier
        ] ?? false;
    ...
    const handleGlobalClick = (event: MouseEvent) => {
      if (contextMenuState.open || slotMenuState.open) {
        ...
        const isInsideMenu =
          ...
          target?.closest("[data-camera-player]");
    ...
    <MenuContext.Provider value={contextValue}>
      ...
      {menuState.open && menuState.camera && (
        <Menu
          ...
          BackdropProps={{
            style: {
              zIndex: isFullscreen ? 9003 : isPlayerFullscreen ? 8099 : 1000,
            },
          }}
          style={{
            zIndex: isFullscreen ? 9004 : isPlayerFullscreen ? 8100 : 1001,
        ...
        >
          ...
          {currentLayout !== "auto" && (
            <MenuItem
              ...
              disabled={
                layoutConfig.mainSlot === contextMenuState.camera?.identifier
              }
            >
              <ListItemIcon sx={{ minWidth: "auto" }}>
      ...
      {slotMenuState.open && contextMenuState.camera && (
        ...
        >
          {Object.values(filteredCameras)
            .filter(
              (camera) =>
                camera.identifier !== contextMenuState.camera?.identifier,
            )
            .map((camera) => (
              <MenuItem
...
export const FloatingMenu = memo(
  ({
    ...
    const [hideState, setHideState] = useState({
      shouldHide: false,
      fullscreenKey: isFullscreen,
    });

    // Auto-reset state when fullscreen changes (derived state pattern)
    const currentShouldHide =
      hideState.fullscreenKey !== isFullscreen ? false : hideState.shouldHide;
    ...
    return (
      <>
        ...
        >
          ...
          >
            <Fab size="small" color="primary" onClick={handleFullscreenToggle}>
