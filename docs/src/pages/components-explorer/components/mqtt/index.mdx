...
A JPEG encoded image is published to this topic when a post processor stores a snapshot, where `{domain}` is one of `face_recognition`, `image_classification` or `license_plate_recognition`. The image is identical to the one saved to disk, including any zoom and annotations.
...
2. A camera entity named `camera.{camera_identifier}_latest_{domain}_snapshot` is created for each configured post processor, showing its most recent snapshot.
