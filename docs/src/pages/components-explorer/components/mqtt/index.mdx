...
The MQTT component can be used as a motion detector by using the following configuration:
...
mqtt:
  ...
  motion_detector:
    cameras:
      camera_one:
        topic: viseron/camera_one/motion
        payload_on: "on"
        payload_off: "off"
        max_motion_duration: 60
```

The motion detector will subscribe to the specified topic and trigger motion when a message with the `payload_on` is received.
Motion will be considered ended when a message with the `payload_off` is received or when the `max_motion_duration` is reached.

The MQTT motion detector can be used in the exact same way as other motion detectors in Viseron, but with the added benefit of being able to utilize any method of detecting motion that can publish to MQTT, such as a PIR sensor or similar.
Since it is not analyzing the video stream, it will also use very little resources.
...
#### `{base_topic}/image/{camera_identifier}_latest_{domain}_snapshot/image`

A JPEG encoded image is published to this topic when the object detector or a post processor stores a snapshot, where `{domain}` is one of `object_detector`, `face_recognition`, `image_classification` or `license_plate_recognition`. The image is identical to the one saved to disk, including any zoom and annotations.

#### `{base_topic}/image/{camera_identifier}_latest_{domain}_snapshot/attributes`

A JSON formatted payload with the path of the snapshot that was saved, the filename the latest snapshot is stored under, and when it was last updated.
...
2. A camera entity named `camera.{camera_identifier}_latest_{domain}_snapshot` is created for the object detector and each configured post processor, showing its most recent snapshot.
