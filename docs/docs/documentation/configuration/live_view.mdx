...
If you have not configured the [go2rtc component](/components-explorer/components/go2rtc), the live view will use the MJPEG stream by default. This is a fallback option that provides a live feed but with lower quality and higher latency compared to WebRTC or MSE.
...
Viseron utilizes [go2rtc](https://github.com/AlexxIT/go2rtc) to provide WebRTC and MSE support for high-quality, low-latency streams through the Viseron [go2rtc component](/components-explorer/components/go2rtc).
...
Example configuration can be found in the [go2rtc component documentation](/components-explorer/components/go2rtc).
...
Cameras with PTZ support through the [ONVIF component](/components-explorer/components/onvif) get a PTZ button in the video player controls.
Clicking it opens a draggable card that lets you pan, tilt and zoom the camera, adjust the movement speed and manage presets and the home position.
The PTZ controls are only available to admin users.

<details>
  <summary>PTZ controls (highlighted in green)</summary>
