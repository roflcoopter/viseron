...
Builds are published to [Docker Hub](https://hub.docker.com/u/roflcoopter?page=1&search=viseron).
...
- ✅ Correct: `proxy_pass http://localhost:8888/;`
- ❌ Wrong: `proxy_pass http://localhost:8888/viseron/;`
...
1. `config.yaml`: `webserver: { subpath: "/viseron" }`
2. External reverse proxy: `location /viseron/ { ... }`
...
If you want to skip recursive `chown` operations on volume paths during startup,
set `VISERON_DISABLE_CHOWN=true`.
This can make startup faster on large mounts, but then ownership of your mounted
paths must already be correct on the host.

When enabled, Viseron skips recursive ownership changes for:
...
- `/snapshots`
- `/thumbnails`
- `/data/*` (Home Assistant add-on data mount mode)
...
Viseron uses a PostgreSQL database to store all the data needed for the application.
By default, the database runs within the container and is not exposed to the host machine.

You can however point Viseron to an external database by setting the `POSTGRES_DATABASE_URL` environment variable.
...
Viseron can be installed as an app for users running Home Assistant OS.

In Home Assistant, perform the following actions:

- Navigate to `Settings` > `Apps` > `App Store` > `Repositories`
- Add `https://github.com/roflcoopter/viseron-hass-app`
- Install the latest version of the Viseron app from the `App Store` and wait for it to finish
- Start the App
- Click `Open Web UI` button to access the frontend
- Click `Settings` in the sidebar, then `Configuration` to access the configuration editor
