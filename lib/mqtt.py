...
MQTT_SWITCH_CONFIG = "%s/switch/%s/config" % (
    config.MQTT_PREFIX, config.MQTT_TOPIC)
MQTT_SWITCH_CMD = "%s/switch/%s/set" % (config.MQTT_PREFIX, config.MQTT_TOPIC)
MQTT_SWITCH_STATE = "%s/switch/%s/state" % (
    config.MQTT_PREFIX, config.MQTT_TOPIC)

MQTT_SENSOR_CONFIG = "%s/sensor/%s/config" % (
    config.MQTT_PREFIX, config.MQTT_TOPIC)
MQTT_SENSOR_AVAILABILITY = "%s/sensor/%s/lwt" % (
    config.MQTT_PREFIX, config.MQTT_TOPIC)
MQTT_SENSOR_STATE = "%s/sensor/%s/state" % (
    config.MQTT_PREFIX, config.MQTT_TOPIC)

MQTT_CAMERA_CONFIG = "%s/camera/%s/config" % (
    config.MQTT_PREFIX, config.MQTT_TOPIC)
MQTT_CAMERA_IMAGE = "%s/camera/%s/image" % (
    config.MQTT_PREFIX, config.MQTT_TOPIC)
...
class MQTT(object):
    ...
    def on_connect(client, userdata, flags, rc):
        ...
        # Switch config message/subscription
        payload = {}
        payload['name'] = config.MQTT_TOPIC
        payload['command_topic'] = MQTT_SWITCH_CMD
        payload['state_topic'] = MQTT_SWITCH_STATE
        payload['retain'] = True
        json_payload = json.dumps(payload, indent=3)
        client.publish(MQTT_SWITCH_CONFIG, payload=json_payload, retain=True)
...
#    self.client.on_publish = MQTT.on_publish
        self.client.enable_logger(logger=logging.getLogger(__name__))
        logging.getLogger('lib.mqtt').setLevel(logging.ERROR)
        if config.MQTT_USERNAME:
            self.client.username_pw_set(
                config.MQTT_USERNAME, config.MQTT_PASSWORD)

        # Set a Last Will message
        self.client.will_set(MQTT_SENSOR_AVAILABILITY,
                             payload="dead", retain=True)
    ...
    def publish_sensor(self, object_detected, detections):
        ...
        self.client.publish(MQTT_SENSOR_STATE,
