...
class ObjectDetection(object):
    def __init__(self, input, model, config, classes, thr, nms, camera_res):
        self.input = input
    ...
    def postprocess(self, frame, outs):
        # TODO saving this for future reference to provide scaled and unscaled boxes
        try:
            frameHeight = frame.shape[0]
            frameWidth = frame.shape[1]
        except AttributeError:
            frameHeight = self.camera_res[1]
            frameWidth = self.camera_res[0]
    ...
    def return_objects(self, frame):
        ...
        inpWidth = 320
        inpHeight = 320
        blob = cv.dnn.blobFromImage(frame, 0.00392, (inpWidth, inpHeight), [
...
def parse_args():
    ...
    parser.add_argument(
        '--classes', help='Optional path to a text file with names of classes to label detected objects.')
