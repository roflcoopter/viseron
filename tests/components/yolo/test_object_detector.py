...
from tests.components.yolo.common import (
    DetectionBuilder,
    DetectionSpec,
    ErrYOLO,
    FakeYOLO,
...
def patch_yolo(monkeypatch: MonkeyPatch) -> None:
    ...
    def fake_init(
        ...
        # Get the camera from the vis registry
        self._camera = vis.get_registered_domain("camera", camera_identifier)
        # Register the object detector domain to avoid registry errors
        vis._domain_registry.register(
            component_name=component,
            component_path=f"viseron.components.{component}",
...
def test_postprocess_creates_valid_bounding_boxes(detector: od.ObjectDetector):
    """Postprocess should create valid bounding boxes."""
    result = DetectionBuilder().add_spec(PERSON_DETECTION).build()
